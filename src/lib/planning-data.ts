...
const getEditorialData = (): ProgramStatus[] => {
  const baseData: Partial<ProgramStatus>[] = [
    { id: 'ED-1', cct: '15DES0002Q', agrupado: 'DESMEXICO09024', vertiente: 'DES', sector: '09', zonaEscolar: '024', fechaAlta: '2022/10/19', fechaModif: '2022/10/20', fechaRevision: '2025/09/01', date: '2023/04/19', status: 'concluido', email: 'des0002q@desysa.gob.mx', observaciones: '' },
    ...
    { id: 'ED-827', cct: '15DES0072L', agrupado: 'DESMEXICO02040', vertiente: 'DES', sector: '02', zonaEscolar: '040', fechaAlta: '2025/02/07', fechaModif: '2025/02/07', fechaRevision: '2025/02/07', date: '2025/02/07', status: 'concluido', email: 'des0072l@desysa.gob.mx', observaciones: '' }
  ];

  const results: ProgramStatus[] = baseData.map(item => ({
    ...item,
    name: 'Conoce mi Escuela',
    progress: 100,
  } as ProgramStatus));

  // Loop to reach 827 items for demonstration (using data provided in turns)
  for (let i = results.length + 1; i <= 827; i++) {
    ...
    const cct = `${vert}${zona}${String.fromCharCode(65 + (i % 26))}`;
