...
// DATA DUMMY PARA CONOCE MI ESCUELA
const conocemiEscuelaInit: ProgramStatus[] = [
...
export const programsData: ProgramStatus[] = [
  ...
  ...MASTER_RAW_DATA.map((r, i) => ({
    ...
    valle: r[2].toUpperCase().includes('MEXICO') ? 'MÉXICO' : r[2].toUpperCase(),
    ...
    asistentes: [{
      ...
      rfc: '',
      genero: '',
      funcion: r[4] === 'ADMIN' ? 'ADMINISTRATIVO' : 'RESPONSABLE',
      email: r[6].trim(),
      cct: r[6].split('@')[0].trim().toUpperCase(),
      nombreCT: r[4] === 'ADMIN' ? 'OFICINA CENTRAL' : ` DEPENDENCIA / PLANTEL ${r[3]}`,
      ze: r[5],
      sector: r[0],
      modalidad: r[1].toUpperCase(),
      municipio: 'EDOMÉX',
