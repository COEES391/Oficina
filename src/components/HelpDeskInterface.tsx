    ...
    const initSupportSession = async () => {
      if (isPublic) {
        ...
        try {
          ...
          console.error("Falla heartbeat:", e);
        }
      } else {
      ...
      <div className="flex h-full w-full bg-[#f4f7f9] overflow-hidden">
        <aside className="hidden lg:flex w-[280px] bg-[#1a2b4b] flex-col shrink-0 p-6 shadow-2xl z-30">
          ...
          <div className="mt-auto">
             <div className="bg-[#2a3c5d] p-5 rounded-[1.8rem] border border-white/5 space-y-4 shadow-xl">
                ...
                <Button className="w-full bg-[#0052cc] hover:bg-[#0047b3] text-white rounded-xl h-10 text-[10px] font-black gap-2 shadow-lg"><Headphones className="h-4 w-4" /> Hablar con técnico</Button>
        ...
        <div className="flex-1 flex flex-col overflow-hidden relative">
          <header className="h-16 bg-white border-b flex items-center justify-between px-8 shrink-0 z-20 shadow-sm">
             <div className="flex items-center gap-3"><h2 className="text-sm font-black text-slate-700 uppercase">Asistente COEES</h2><Badge className="bg-emerald-50 text-emerald-600 border-none text-[8px] font-black uppercase px-2 h-5 rounded-full"><Circle className="h-1.5 w-1.5 fill-current mr-1.5" /> Chat Seguro</Badge></div>
             <div className="flex items-center gap-6"><div className="flex items-center gap-2"><Clock className="h-4 w-4 text-slate-300" /><div><p className="text-[8px] font-black text-slate-400 uppercase">Atención</p><p className="text-[10px] font-bold text-slate-600">8:00 - 4:00 p.m.</p></div></div><div className="h-8 w-px bg-slate-100" /><p className="text-[9px] font-black text-primary uppercase">Folio: {sessionKey.split('-').at(-1)}</p></div>
          </header>
             ...
             <div className="max-w-4xl mx-auto space-y-10 pb-24">
                ...
                <div className="space-y-8 pt-4">
                   ...
                   <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
                      {[ { id: 'pass', label: 'Contraseña', icon: Lock, color: 'text-blue-600', bg: 'bg-blue-50', sub: 'Acceso' }, { id: 'mail', label: 'Correo', icon: Mail, color: 'text-emerald-600', bg: 'bg-emerald-50', sub: 'Institucional' }, { id: 'tech', label: 'Soporte', icon: Laptop, color: 'text-orange-500', bg: 'bg-orange-50', sub: 'Técnico' }, { id: 'lib', label: 'Biblioteca', icon: BookOpen, color: 'text-purple-600', bg: 'bg-purple-50', sub: 'Digital' }, { id: 'edu', label: 'Plataforma', icon: GraduationCap, color: 'text-cyan-600', bg: 'bg-cyan-50', sub: 'Moodle' }, { id: 'other', label: 'Otro', icon: MoreHorizontal, color: 'text-rose-500', bg: 'bg-orange-50', sub: 'Consulta' } ].map((cat) => (
                ...
                <div className="flex items-center gap-4 bg-blue-50/50 p-4 rounded-2xl border border-blue-100"><Info className="h-5 w-5 text-[#0052cc] shrink-0" /><p className="text-[10px] font-bold text-slate-500 uppercase leading-relaxed">Si no encuentras la opción que necesitas, escríbenos tu problema en el cuadro de abajo y te ayudaremos de inmediato.</p></div>
    ...
    <div className="flex h-full w-full overflow-hidden bg-white">
      <div className="w-[280px] bg-[#0b4135] flex flex-col shrink-0 overflow-hidden shadow-2xl z-40">
        <div className="p-6 flex-1 flex flex-col overflow-hidden">
          <div className="flex items-center gap-4 mb-8"><div className="h-12 w-12 rounded-2xl bg-white/10 flex items-center justify-center text-emerald-400 border border-white/5"><Bot className="h-7 w-7" /></div><div className="min-w-0"><h3 className="text-white font-black uppercase text-sm leading-none tracking-tighter">Mesa de Ayuda</h3><div className="flex items-center gap-1.5 mt-1.5"><div className="h-2 w-2 rounded-full bg-emerald-500 animate-pulse" /><span className="text-[9px] font-bold text-emerald-500 uppercase tracking-widest">Sincronizado</span></div></div></div>
          <nav className="space-y-1">
          ...
          <div className="mt-auto space-y-3"><button onClick={() => setIsQrDialogOpen(true)} className="w-full flex items-center gap-3 px-4 py-3 text-emerald-400 hover:text-white transition-all bg-white/5 rounded-xl border border-white/10 shadow-lg group"><QrCode className="h-4 w-4" /><span className="text-[11px] font-black uppercase tracking-widest">Acceso Usuarios (QR)</span></button></div>
        </div>
        <div className="p-6 bg-black/20 flex items-center gap-4 border-t border-white/5"><Avatar className="h-10 w-10 border-2 border-emerald-500 shadow-lg"><AvatarFallback className="bg-emerald-800 text-white font-black text-xs">AN</AvatarFallback></Avatar><div className="min-w-0"><p className="text-xs font-black text-white uppercase truncate">{techName}</p><p className="text-[9px] font-bold text-white/30 uppercase mt-1">Analista Auditor</p></div></div>
      </div>
          ...
          <>
            ...
            <ScrollArea className="flex-1 px-10 py-10 bg-[#efe7dd] bg-[url('https://www.transparenttextures.com/patterns/cubes.png')]">
               <div className="max-w-5xl mx-auto flex flex-col space-y-6">
                  <div className="self-center bg-white/50 backdrop-blur-md px-6 py-2 rounded-full border border-white text-[9px] font-black uppercase text-slate-400 shadow-sm">Hoy, {format(new Date(), 'dd MMMM yyyy')}</div>
                  {messages.map((msg, i) => (
            ...
            <footer className="p-6 bg-slate-50 border-t shrink-0 shadow-lg"><div className="max-w-5xl mx-auto flex items-center gap-4"><button className="h-12 w-12 rounded-2xl bg-white border border-slate-200 text-slate-400 hover:text-emerald-500 flex items-center justify-center shadow-sm"><Paperclip className="h-5 w-5" /></button><Input placeholder="Escribir respuesta técnica..." className="h-12 rounded-2xl bg-white border-none shadow-inner px-6 font-bold text-sm text-slate-700 uppercase focus:ring-2 focus:ring-emerald-500/20" value={input} onChange={e => setInput(e.target.value)} onKeyDown={e => e.key === 'Enter' && handleSendMessage()} disabled={isSending} /><Button onClick={() => handleSendMessage()} disabled={!input.trim() || isSending} className="bg-[#128c7e] hover:bg-[#075e54] h-12 w-12 rounded-2xl shadow-xl p-0 shrink-0">{isSending ? <Loader2 className="h-5 w-5 animate-spin" /> : <Send className="h-5 w-5" />}</Button></div></footer>
          </>
        ) : (
          <div className="flex-1 flex flex-col items-center justify-center p-20 text-center opacity-20 animate-in fade-in duration-1000"><div className="h-40 w-40 rounded-full bg-slate-200 flex items-center justify-center mb-8 shadow-inner border-4 border-white"><MessageSquare className="h-20 w-20 text-slate-400" /></div><h3 className="text-3xl font-black uppercase text-slate-800 tracking-tighter leading-none">Mesa de Ayuda Operativa</h3><p className="text-sm font-bold uppercase tracking-[0.4em] text-slate-500 mt-6 border-y border-slate-300 py-2 px-6">Auditoría COEES • Ciclo 2025-2026</p></div>
        )}
      </div>
      ...
      <Dialog open={isQrDialogOpen} onOpenChange={setIsQrDialogOpen}>
        <DialogContent className="sm:max-w-[450px] w-[95vw] rounded-[3rem] p-0 overflow-hidden border-none shadow-2xl bg-[#0b4135] text-white">
          <DialogHeader className="p-8 text-center bg-white/5 border-b shrink-0"><DialogTitle className="uppercase font-black text-xl flex items-center justify-center gap-3"><QrCode className="h-8 w-8 text-emerald-400" /> Acceso Móvil ATRES</DialogTitle></DialogHeader>
          <div className="px-8 pb-8 pt-8 space-y-6 flex flex-col items-center text-center"><div className="p-6 bg-white rounded-[2.5rem] shadow-2xl w-full max-w-[260px] mx-auto border-4 border-emerald-500/20"><div className="aspect-square relative overflow-hidden rounded-[2rem] border border-slate-100 bg-white"><Image src={`https://api.qrserver.com/v1/create-qr-code/?size=300x300&data=${encodeURIComponent(typeof window !== 'undefined' ? `${window.location.origin}/helpdesk` : "")}`} alt="QR" fill className="object-contain p-4" unoptimized /></div></div><Button onClick={() => { navigator.clipboard.writeText(`${window.location.origin}/helpdesk`); toast({ title: "Enlace Copiado" }); }} className="h-10 px-6 rounded-xl bg-emerald-500 text-white gap-2 shadow-xl"><Copy className="h-4 w-4" /> Copiar Enlace</Button></div>
        </DialogContent>
      </Dialog>
    </div>
  );
