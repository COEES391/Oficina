        ...
        {activeTab !== 'ATRES' && (
          <Card className="executive-card p-4 bg-white/80 border-none shadow-lg">
            <div className="flex flex-col md:flex-row items-end gap-4">
               ...
               <div className="flex items-center gap-2 w-full md:w-auto shrink-0">
                  <div className="space-y-1 min-w-[200px]"><Label className="text-[9px] font-black uppercase text-slate-400 mb-1 block pl-1">Oficina Regional</Label><Select value={officeFilter} onValueChange={setOficinaFilter}><SelectTrigger className="h-10 w-full rounded-xl border-primary/5 bg-white text-[9px] font-black uppercase shadow-sm"><SelectValue placeholder="TODAS" /></SelectTrigger><SelectContent><SelectItem value="all" className="text-[9px] font-black uppercase">TODAS</SelectItem>{REGIONAL_OFFICES.map(off => <SelectItem key={`off-reg-${off}`} value={off} className="text-[9px] font-black uppercase">{off.replace("Oficina de ", "")}</SelectItem>)}</SelectContent></Select></div>
               </div>
            </div>
          </Card>
        )}
