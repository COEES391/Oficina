...
import { Users, UserPlus, Pencil, Trash2, ShieldCheck, Lock, Shield, Save, KeyRound } from "lucide-react"
  ...
  const handleSave = async () => {
    ...
    try {
      if (editingId) {
        ...
        toast({ title: "Nuevo usuario registrado en la nube" })
      }
    ...
    <div className="space-y-6 font-sans">
      <div className="flex justify-between items-center">
        <div className="space-y-1">
          ...
          <p className="text-muted-foreground font-bold text-[10px] uppercase tracking-[0.2em] flex items-center gap-2">
            <ShieldCheck className="h-4 w-4 text-accent" /> Control de Privilegios en la Nube
          </p>
        </div>
      ...
      <Card className="executive-card p-0 overflow-hidden border-t-8 border-t-primary shadow-2xl">
        <CardHeader className="bg-slate-50/50 p-8">
          ...
          <CardDescription className="font-bold text-xs uppercase tracking-[0.2em] text-muted-foreground mt-2">Personal con acceso sincronizado en tiempo real</CardDescription>
        </CardHeader>
        <CardContent className="p-0">
          <Table>
            ...
            <TableBody>
              {isLoading ? (
                <TableRow><TableCell colSpan={4} className="text-center py-20 font-black uppercase opacity-50">Sincronizando con la nube...</TableCell></TableRow>
                ...
                <TableRow><TableCell colSpan={4} className="text-center py-20 opacity-30 font-black uppercase text-[10px]">No hay usuarios en la base de datos central.</TableCell></TableRow>
              )}
            </TableBody>
          </Table>
        </CardContent>
      ...
      <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
        <DialogContent className="sm:max-w-[750px] h-[85vh] max-h-[850px] rounded-[3rem] border-none shadow-2xl p-0 flex flex-col overflow-hidden bg-white">
          <DialogHeader className="p-8 bg-slate-50 border-b shrink-0">
            ...
            <DialogDescription className="font-bold text-[10px] uppercase tracking-widest text-muted-foreground mt-2">Credenciales globales para acceso multi-equipo.</DialogDescription>
          </DialogHeader>
            ...
            <div className="p-8 space-y-8">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="md:col-span-2 space-y-2">
                  ...
                  <Input value={formData.name} onChange={e => setFormData({...formData, name: e.target.value.toUpperCase()})} className="h-12 rounded-2xl bg-slate-50 border-none shadow-inner px-6 text-xs font-black uppercase" />
                </div>
                <div className="space-y-2">
                  <Label className="text-[10px] font-black uppercase text-primary tracking-widest pl-1">Identificador (Usuario)</Label>
                  <Input value={formData.rfc} onChange={e => setFormData({...formData, rfc: e.target.value.toUpperCase()})} className="h-12 rounded-2xl bg-slate-50 border-none shadow-inner px-6 font-mono text-sm text-primary" />
                </div>
                <div className="space-y-2">
              ...
              <div className="space-y-4 pt-4 border-t border-slate-100">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  {SECTIONS.map(section => (
                    <div key={section.id} className={cn("flex items-center space-x-4 p-4 rounded-2xl border transition-all cursor-pointer", formData.privileges.includes(section.id) ? "bg-primary/[0.03] border-primary/20" : "bg-white border-slate-100")} onClick={() => handleTogglePrivilege(section.id)}>
                        ...
                        <Label className="text-[10px] font-black uppercase cursor-pointer">{section.name}</Label>
                    </div>
                  ))}
                </div>
              </div>
          ...
          <DialogFooter className="p-8 bg-slate-50 border-t flex justify-end gap-4 shrink-0">
            <Button variant="ghost" onClick={() => setIsDialogOpen(false)} className="font-black text-[10px] uppercase h-12 px-10 text-slate-400">Cancelar</Button>
            <Button onClick={handleSave} className="btn-institutional h-12 px-14 text-[10px] flex items-center gap-3">
              <Save className="h-5 w-5" /> {editingId ? 'Actualizar' : 'Registrar'}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
